...
<input type="number" min={durationUnit==='hours'?'0.25':'1'} step={durationUnit==='hours'?'0.25':'1'} value={durationUnit==='hours'?Number(durationValue.toFixed(2)):durationValue} onChange={e=>set('duration_minutes',Math.max(1,Number(e.target.value)*(durationUnit==='hours'?60:1)))}/>
<select value={durationUnit} onChange={e=>{const next=e.target.value as 'minutes'|'hours';if(next===durationUnit)return;const converted=next==='hours'?Number((value.duration_minutes/60).toFixed(2)):value.duration_minutes*60;const message=lang==='de'?`Dauer in ${next==='hours'?'Stunden':'Minuten'} umrechnen (${converted} ${next==='hours'?'h':'min'})?`:`Convert duration to ${next} (${converted} ${next==='hours'?'h':'min'})?`;if(confirm(message)){setDurationUnit(next);set('duration_minutes',Math.max(1,converted*(next==='hours'?60:1)))}}} aria-label={t('duration')}><option value="minutes">{t('minutes')}</option><option value="hours">{lang==='de'?'Stunden':'Hours'}</option></select>
</div>
</Field>
